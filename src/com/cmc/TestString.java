...
//		getDepthnames(depthname);
	    ...
	    map2.put("count", 3);
	    map2.put("toid", 123456);
	    Map<String, Object> map3 = new HashMap<>();
	    map3.put("count", 4);
	    map3.put("toid", 1234567);
	    datas.add(map1);
	    datas.add(map2);
	    datas.add(map3);
        ...
        for(int i =0 ; i<datas.size();i++){
            for (int j =0 ;j<datas1.size();j++){
                ...
                if(datas1.get(j).get("toid").toString().equals(datas.get(i).get("toid").toString())){
                    datas.get(i).put("count", Long.valueOf(datas.get(i).get("count").toString()) + Long.valueOf(datas1.get(j).get("count").toString()));
                    System.out.println(datas.get(i).get("toid").toString()+datas.get(i).get("count").toString());
                    datas1.remove(j);
                    break;
                }
            }
		...
		depthnames.add("aaa/bbb/ccc");
		depthnames.add("aaa/bbb");
		depthnames.add("aaa");
		String depthname = "";
		int maxLength = 0;
		for (String name : depthnames) {
...
//	    String s = "mali_901";
	    ...
	    System.out.println("+8613300133555".hashCode());
        ...
        if(openFileFlagList == null || openFileFlagList.size() <1){+8613300133666
            System.out.println("该用户移动端已有打开文件权限");743747820
        }*/
	   /* String s = "/aaa/bbb&a1&a2";
	    char c = 0x5e;
//	    String[] split = s.split(String.valueOf(c));
        ...
        String custumerPath = "";
	    if(split.length == 3){
            managerPath = split[0] + "/" + split[1];
            custumerPath = split[0] + "/" + split[2];
        }else{
            System.out.println("参数传入错误，程序结束！----path" + s);
            System.gc();
            System.exit(0);
        }
